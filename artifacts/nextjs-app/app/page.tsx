  ...
  return (
    <div className="bg-white">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-5">
            <Link href="/browse">
              <Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 h-12 rounded-lg gap-2 text-base shadow-sm">
                <CalendarDays className="w-5 h-5" /> Find &amp; Book Now →
              </Button>
            </Link>
            <Button size="lg" variant="outline" className="border-2 border-blue-600 text-blue-600 hover:bg-blue-50 font-semibold px-8 h-12 rounded-lg text-base" onClick={() => router.push("/for-business")}>
              List Your Factory
            </Button>
          </div>
          <Link href="/ai-assistant">
            <Button size="lg" className="bg-violet-600 hover:bg-violet-700 text-white font-semibold px-8 h-12 rounded-lg gap-2 text-base shadow-sm">
              <Sparkles className="w-5 h-5" /> Try AI Manufacturing Assistant (Free)
            </Button>
          </Link>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <SponsoredAds placement="HOMEPAGE" className="max-w-md mx-auto" />
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {STEPS.map(s => (
              <div key={s.title} className="flex flex-col items-center text-center">
                <div className={`w-16 h-16 rounded-full ${s.bg} flex items-center justify-center mb-5`}>{s.icon}</div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="text-white">
              ...
              <h2 className="text-4xl font-bold leading-tight mb-5">Turn Ideas into Reality with AI Guidance</h2>
              <p className="text-blue-100 text-base leading-relaxed mb-6">Not sure where to start? Our AI Assistant analyzes your product idea and creates a complete manufacturing roadmap with:</p>
              <ul className="space-y-3 mb-8">
                {["Layered manufacturing process breakdown", "Required machines and processes for each phase", "Material and product recommendations", "Cost estimates and timeline projections"].map(item => (
                  <li key={item} className="flex items-start gap-2 text-blue-100 text-sm">
                    <CheckCircle2 className="w-4 h-4 text-blue-300 shrink-0 mt-0.5" /> {item}
                  </li>
                ))}
              </ul>
              <Link href="/ai-assistant">
      ...
      <footer className="bg-[#0F172A] text-white py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-10">
            <div>
              <div className="flex items-center gap-2 mb-3"><span className="text-2xl font-bold text-white">X!Y</span></div>
              <p className="text-gray-400 text-sm">The Explorer Factory</p>
            </div>
            <div>
              <h4 className="font-semibold text-white mb-4">Platform</h4>
              <ul className="space-y-2 text-gray-400 text-sm">
                <li><Link href="/browse" className="hover:text-white">Find Manufacturers</Link></li>
                <li><Link href="/for-business" className="hover:text-white">List Your Factory</Link></li>
              </ul>
            </div>
            <div>
